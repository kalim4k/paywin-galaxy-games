...
export const useDynamitGame = () => {
  ...
  const startGame = async () => {
    ...
    try {
      ...
      await updateBalance(
        ...
        'game_loss',
        bet,
        `Mise au jeu Dynamit - ${bombs} bombes`
      );
    } catch (error) {
      setIsPlaying(false);
  ...
  const handleCellClick = useCallback((index: number) => {
    ...
    if (shouldBeBomb) {
      ...
      toast.error(`💥 BOOM ! Vous avez touché une dynamite !`, {
  ...
  const cashOut = async () => {
    ...
    try {
      ...
      await updateBalance(
        ...
        'game_win',
        netGain,
        `Gain au jeu Dynamit - ${revealedStars} étoiles trouvées`
      );
    } catch (error) {
